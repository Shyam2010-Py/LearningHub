...
const STATIC_ASSETS = [
  ...
  './js/hub.js', './js/activity.js', './js/auth.js', './js/my-learning.js', './js/supabase.js', './js/handoff.js',
...
self.addEventListener('install', event => {
  ...
  event.waitUntil(caches.open(STATIC_CACHE).then(cache => Promise.all(
    STATIC_ASSETS.map(url => cache.add(url).catch(err => console.warn('[SW] Skipped:', url, err)))
  )));
});
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys().then(keys => Promise.all(
      keys.filter(key => ![STATIC_CACHE, HTML_CACHE, RUNTIME_CACHE].includes(key)).map(key => caches.delete(key))
...
async function cacheFirst(req) {
  ...
  try {
    ...
    if (req.destination === 'image') return new Response('<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"/>', { headers: { 'Content-Type': 'image/svg+xml' } });
    throw error;
  }
}
